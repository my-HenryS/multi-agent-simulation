...
public class ApplicationForECStrategy extends ApplicationForECTest implements SocialForceApplication{
    ...
    public void setUpScenes(){
        ...
        values.add(new SVSR_RandomAgentGenerator(400,new Box2D(4,4 ,27.5,15.5)));
        values.add(new SVSR_SafetyRegion(new Box2D(24,2,4,1)));
        values.add(new SVSR_SafetyRegion(new Box2D(33,12,1,4)));
        values.add(new SVSR_SafetyRegion(new Box2D(2,8,1,4)));
        values.add(new SVSR_SafetyRegion(new Box2D(12,21,4,1)));
        loader.readValueSet(values);
